...
public class BarcodeGraphicTracker extends Tracker<Barcode> {
    ...
    public BarcodeGraphicTracker(GraphicOverlay<BarcodeGraphic> mOverlay, BarcodeGraphic mGraphic,
                                 Context context, IPropertyURLResolver resolver) {
    ...
    public void onNewItem(int id, Barcode item) {
        // check is a domain url
        if (!mResolver.isResolvable(item.rawValue)) {
            Snackbar.make(mOverlay, "Not a valid property QR code", Snackbar.LENGTH_SHORT).show();
            return;
        }
        mGraphic.setId(id);
